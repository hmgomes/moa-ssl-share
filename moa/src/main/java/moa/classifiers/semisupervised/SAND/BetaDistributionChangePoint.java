    ...
    public int/*estimated change point*/ detectChange() throws Exception
    {
        ...
        for(int k = this.cushion; k <= N-this.cushion; k++)
        {
            if(calculateMean(k, N-1) <= 0.95*calculateMean(0, k-1))
            {
                ...
                BetaDistribution preBetaDist = new BetaDistribution(alphaPreChange, betaPreChange);
                BetaDistribution postBetaDist = new BetaDistribution(alphaPostChange, betaPostChange);
