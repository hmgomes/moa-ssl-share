 ...
 * @version $Revision: 1 $
 */
public class StreamingRandomPatchesSurvey extends AbstractClassifier implements MultiClassClassifier, SLEADBaseEnsemble /*,
        CapabilitiesHandler*/ {
    ...
    public FileOption debugOutputFileOption = new FileOption("debugOutputFile", 'g',
            "File to append debug information.", null, "csv", true);


    public ClassOption studentLearnerForUnsupervisedDriftDetectionOption = new ClassOption("studentLearnerForUnsupervisedDriftDetection", 'b',
...
//                        "Random seed for random number generation.", 1);
    ...
    protected PrintStream outputDebugStream;


    Classifier student;

//    public boolean changeDetectionUpdateAllowed = true;
    protected ChangeDetector driftDetectionMethod;


    private boolean trainAndUpdateStudentDetector(Instance instance) {
        double[] ensembleVotes = this.getVotesForInstanceInternal(instance);
...
//            this.numberOfUnsupervisedDriftsDetected++;
            ...
            this.outputDebugStream.println(this.instancesSeen + ", " + this.instancesSeen + ", ???, StudentDrift");
...
//        ++this.instancesSeen;
    ...
    private double[] getVotesForInstanceInternal(Instance instance) {
...
//        testInstance.setClassValue(0.0);
    ...
    public double[] getVotesForInstance(Instance instance) {
        ++this.instancesSeen;
        double votes[] = getVotesForInstanceInternal(instance);
...
//        this.classifierRandom.setSeed(randomSeedOption.getValue());
        ...
        classificationEvaluator.toString();
        ...
        if(this.trainingMethodOption.getChosenIndex() != StreamingRandomPatchesSurvey.TRAIN_RESAMPLING) {
            ...
            switch(this.subspaceModeOption.getChosenIndex()) {
                case StreamingRandomPatchesSurvey.FEATURES_SQRT:
                    k = (int) Math.round(Math.sqrt(n)) + 1;
                    break;
                case StreamingRandomPatchesSurvey.FEATURES_SQRT_INV:
                    k = n - (int) Math.round(Math.sqrt(n) + 1);
                    break;
                case StreamingRandomPatchesSurvey.FEATURES_PERCENT:
            ...
            if(this.trainingMethodOption.getChosenIndex() == StreamingRandomPatchesSurvey.TRAIN_RANDOM_SUBSPACES ||
                    this.trainingMethodOption.getChosenIndex() == StreamingRandomPatchesSurvey.TRAIN_RANDOM_PATCHES) {
                if(k != 0 && k < n) {
                    ...
                    if(n <= 20 || k < 2) {
                        ...
                        this.subspaces = StreamingRandomPatchesSurvey.allKCombinations(k, n);
                    ...
                    // subspaces without worrying about repetitions.
                        ...
                        this.subspaces = StreamingRandomPatchesSurvey.localRandomKCombinations(k, n,
                ...
                // k == 0 or k > n (subspace size greater than the total number of features), then default to resampling
                    ...
                    this.trainingMethodOption.setChosenIndex(StreamingRandomPatchesSurvey.TRAIN_RESAMPLING);
                }
            }
        }
        ...
        for(int i = 0 ; i < ensembleSize ; ++i) {
            switch(this.trainingMethodOption.getChosenIndex()) {
                case StreamingRandomPatchesSurvey.TRAIN_RESAMPLING:
                    this.ensemble[i] = new StreamingRandomPatchesClassifier(
                            ...
                            false);
                    break;
                case StreamingRandomPatchesSurvey.TRAIN_RANDOM_SUBSPACES:
                case StreamingRandomPatchesSurvey.TRAIN_RANDOM_PATCHES:
        ...
        if (this.debugOutputFileOption.getName() != null) {
            //File for debug
            ...
            if (this.outputDebugStream != null)
                for (int i = 0; i < this.ensemble.length; ++i) {
                    this.ensemble[i].outputDebugStream = this.outputDebugStream;
                }

            // Even more Gambi!
            this.outputDebugStream.print("#instances,#labeled_instances,windowed_accuracy(w=???)," + "drift_descriptor,\n");
        }
        /*** TO DEBUG DELAYED DETECTIONS ***/
    }
...
//    }
    ...
    public class StreamingRandomPatchesClassifier implements SLEADBaseClassifier {
        ...
        // To be set by another class. Very dangerous design choice!
        public PrintStream outputDebugStream;
