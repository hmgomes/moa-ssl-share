...
public class Dstream extends AbstractClusterer {
    ...
    public int countPruning;
    public long timePruning;
    ...
    public Clustering getMicroClusteringResult() {
        Cluster[] result = new Cluster[grid_list.keySet().size()];
        this.grid_list.keySet().toArray(result);
    ...
    public void trainOnInstanceImpl(Instance inst) {
        ...
        // 2. Determine the density grid g that contains
        g = new int[this.d];
        for (int i = 0 ; i < this.d ; i++)
        {
            if (inst.attribute(i).isNumeric())
...
//            dg.updateLabelWeight(inst.classValue(), 1);
        ...
        if(!this.grid_list.containsKey(dg))
        {
            ...
            {
                cv = new CharacteristicVector(this.getCurrTime(),
                        this.deleted_grids.get(dg),1.0, -1, false, this.getDL(), this.getDM());
                this.deleted_grids.remove(dg);
            }
            else
                cv = new CharacteristicVector(this.getCurrTime(),
                        -1, 1.0, -1, false, this.getDL(), this.getDM());

            this.grid_list.put(dg, cv);
        }
        else // 4. Update the characteristic vector of dg
        {
            // get the old density grid in the hash map & update its label count
            DensityGrid k = updateLabelCountInGrid(dg);
            ...
            //grid_list.put(dg, cv);
            if (k != null) grid_list.put(k, cv); // put the density grid with the updated count
        }
    ...
    /**
     * Updates the label count of the existing entry k of density grid in the hash map
     * (by accumulating the count in k with those in d)
     * @param d the new density grid
     * @return the existing density grid that has been updated, null if no key is found equal to the given grid
     */
    private DensityGrid updateLabelCountInGrid(DensityGrid d) {
    ...
    private void initialClustering() {
        ...
        HashMap<DensityGrid, CharacteristicVector> newGL = new HashMap<>();
        ...
        while(glIter.hasNext())
        {
            ...
            if(cvOfG.getAttribute() == DENSE)
            {
                ...
                GridCluster gc = new GridCluster(dg, new ArrayList<>(), gridClass);
                gc.addGrid(dg);
     ...
     *
     * @see Dstream#grid_list
     * @see DensityGrid#toString()
     */
     ...
     *
     * @see Dstream#cluster_list
     * @see GridCluster#toString()
     */
