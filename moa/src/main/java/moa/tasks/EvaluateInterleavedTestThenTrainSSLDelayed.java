...
public class EvaluateInterleavedTestThenTrainSSLDelayed extends SemiSupervisedMainTask {
    ...
    protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
        ...
        long delayedInstancesProcessed = 0;
        ...
        while (stream.hasMoreInstances()
            ...
            // labelled data to the
            // learner even in trainOnUnlabeled data to generate statistics such as number
            // of correctly
            ...
            // TRAIN
            // Exhaust the delay buffer
            while (delayBuffer.size() > 0
                    && delayBuffer.get(0).getKey() == instancesProcessed + delayedInstancesProcessed) {
                delayedInstancesProcessed++;
            ...
            if (is_labeled) {
                ...
                // If the learner is a semi-supervised learner, then we can
                // use the unlabeled instance for training.
                if (learner instanceof SemiSupervisedLearner)
                    ((SemiSupervisedLearner) learner).trainOnUnlabeledInstance((Instance) unlabeledExample.getData());

                // Either way, we will use the delayed instance for training
                // even if the learner is not a semi-supervised learner.
                if (delayLength >= 0) {
                    delayBuffer.add(
                            new MutablePair<Long, Example>(
                                    1 + instancesProcessed + delayedInstancesProcessed + delayLength,
                                    originalExample));
                }
            }
