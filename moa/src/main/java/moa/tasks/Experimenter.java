 ...
 * and datasets.
 *
 * @author Heitor Murilo Gomes (hmgomes at waikato dot ac dot nz)
 * @version $Revision: 7 $
 */
public class Experimenter extends AuxiliarMainTask {
...
//    		+ "EvaluatePrequential = 1,2,4,6,7,8;", "1,2,4,6,7,8");
    public MultiChoiceOption evaluationMethodOption = new MultiChoiceOption(
            ...
            new String[]{"Classification Prequential", "Classification Prequential Delayed", "Classification Prequential Batch", "Classification PrequentialCV", "Classification Prequential DelayedCV" , "Regression Prequential",
                    "MultiTarget Prequential", "MultiLabel Prequential", "Classification Test-Then-Train SSL"},
            new String[]{"EvaluatePrequential", "EvaluatePrequentialDelayed", "EvaluatePrequentialBatch", "EvaluatePrequentialCV", "EvaluatePrequentialDelayedCV", "EvaluatePrequentialRegression",
                    "EvaluatePrequentialMultiTarget", "EvaluatePrequentialMultiTarget with a multilabel evaluator", "ClassificationTTTSSL"}, 0);
    ...
    public FlagOption debugLEADOption = new FlagOption("debugLEAD", 'l', "When active it will yield extra debug for LEAD.");

    public FlagOption debugSupervisedInSSL = new FlagOption("debugSupervisedInSSL", 'v', "When active it will deactivate the SSL specific metrics.");
    ...
    protected ArrayList<String> classifiers = new ArrayList<>();
    protected ArrayList<String> classifiersAlias = new ArrayList<>();
    protected ArrayList<String> streams = new ArrayList<>();
    protected ArrayList<String> streamsAlias = new ArrayList<>();
    ...
    protected class Result {
        ...
        protected ArrayList<ArrayList<ArrayList<Double>>> results = new ArrayList<>();
        protected ArrayList<ArrayList<ArrayList<Double>>> ranks = new ArrayList<>();
        protected ArrayList<String> lineIdentifiers = new ArrayList<>();
        ...
        public void addResultLine(String rowID, ArrayList<LearningCurve> learningCurves, int[] metricsIndexes,
            ...
            ArrayList<ArrayList<Double>> resultLine = new ArrayList<>();
            ...
            for (int i = 0; i < this.metrics.length; ++i) {
                ArrayList<Double> resultPerMetric = new ArrayList<>();
                ...
                for (int c = 0; c < learningCurves.size(); ++c) {
                    ...
                    for (int l = 0; l < learningCurves.get(c).numEntries(); ++l) {
                        try {
                            ...
                            System.err.println("The requested metric " + metricsIndexes[i] + " is not available for learner. Ignoring... ");
                        }
                    }
...
//    	String task = this.evaluationMethodOption.getChosenLabel();
        ...
        switch (this.evaluationMethodOption.getChosenIndex()) {
            ...
            case 8:
                taskStr += " -e BasicClassificationPerformanceEvaluator";
//                metricsIndexes = new int[]{1, 2, 5, 6, 7, 8, 11, 12}; Temporally removing ram hours, kappa m, kappa t, ...
//                metricsIndexes = new int[]{1, 5, 11, 12};
                if(this.debugLEADOption.isSet()) {
                    metricsIndexes = new int[]{1, 2, 3, 5, 6, 7, 8, 11, 12, 13, 14, 15, 16, 17, 18};
                } else { // 11, 12, 13 = pseudo-label analysis
                    metricsIndexes = new int[]{1, 2, 3, 5, 6, 7, 8, 11, 12, 13}; // used for the majority of the SSL experiments...
                }
                // Very specific case: SSL evaluation, but we are using only a Supervised algorithm, like ARF.
                // These algorithms do not yield metrics such as #pseudo_labeled, so we need to remove these
                // From the metricIndexes
                if(this.debugSupervisedInSSL.isSet()) {
                    metricsIndexes = new int[]{1, 2, 3, 5, 6, 7, 8}; // used for the majority of the SSL experiments...
                }
                break;
            default:
                break;
