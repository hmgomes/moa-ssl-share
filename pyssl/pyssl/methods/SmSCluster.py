...
from pyssl.ssl_classifier import SemiSupervisedClassifier, SemiSupervisedLabel
...
class MCI_Kmeans:
    ...
    def fit(self, X, Y):
        ...
        while not self._has_converged(
            self.centroids, prev_centroids, self.tol, iters, self.max_iter
        ):
    ...
    def _assign_labels(self, X, centroids):
        # This is the assigned cluster label, which cluster the sample belongs
        # to. Not real class labels.
    ...
    def _get_cluster_samples(self, X, labels):
        # Output which samples are in each cluster. At the same time, the label
        # of the sample is also assigned to the cluster.
        self.cluster_samples = {}
            ...
            result_dict = {
                element: sum(
                    [
                        ...
                        if key != element and key != -1
                    ]
                )
                for element in n_labels
            }
            result_dict[-1] = 0
    ...
    def extract_summary(self):
        # Extract summaries for each cluster. The concept evolution was also
        # considered in the original text, but since I focused on binary
        # classification, this function was not implemented.
        for i in range(self.k):
    ...
    def predict(self, X_test):
        ...
        for i in range(N):
            distances = np.sqrt(
                np.sum((np.array(self.labeled_center) - X_test[i]) ** 2, axis=1)
            )
...
class _SmSCluster:
    def __init__(self, window_size=200, K=5, L=10, Q=5):
...
class SmSCluster(SemiSupervisedClassifier):
    """Wrapper to add river support to the _SmSCluster class."""

    def __init__(self, **kwargs) -> None:
        super().__init__()
        self._sms_cluster = _SmSCluster(**kwargs)

        self.x_shape = None

    def learn_one(self, x: dict, y: SemiSupervisedLabel) -> "SmSCluster":
        sk_x, sk_y = convert_river_to_skmf(x, y)

        if self.x_shape is None:
            self.x_shape = sk_x.shape
        assert (
            self.x_shape == sk_x.shape
        ), "The shape of the input data is not consistent with the previous data."

        self._sms_cluster.partial_fit(sk_x, sk_y)
        return self

    def predict_proba_one(self, x: dict) -> t.Dict[river_typing.ClfTarget, float]:
        sk_x, _ = convert_river_to_skmf(x, None)

        array_proba = self._sms_cluster.predict_proba(sk_x)

        # Convert to dict with target as key
        dict_proba = {}
        for i in range(len(array_proba)):
            dict_proba[i] = array_proba[i]
        return dict_proba
