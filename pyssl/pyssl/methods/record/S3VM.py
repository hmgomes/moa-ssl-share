...
class S3VM(object):
    def __init__(
        self, kernel="linear", Cl=1.5, Cu=0.001, gamma="scale", C=1, max_iter=1000
    ):
        """
        Initial TSVM
    ...
    def fit(self, X1, Y1, X2):
        ...
        X1: Input data with labels
                np.array, shape:[n1, m], n1: numbers of samples with labels,
                m: numbers of features
        Y1: labels of X1
                np.array, shape:[n1, ], n1: numbers of samples with labels
        X2: Input data without labels
                np.array, shape:[n2, m], n2: numbers of samples without labels,
                m: numbers of features
        """
        N = len(X1) + len(X2)
        ...
        while self.Cu < self.Cl:
            ...
            while iter < self.max_iter:
                ...
                # print(Y2)
                if positive_id.shape[0] == 0 or negative_id.shape[0] == 0:
                    break
