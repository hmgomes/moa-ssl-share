...
class LogisticRegression(nn.Module):
    def __init__(self, input_dims, output_dims, n_hidden=None):
        ...
        if n_hidden is None:
            n_hidden = int((input_dims + output_dims) * 3 / 2)
...
class MT:
    ...
    def fit(self, X, uX, _y):
        ...
        for epoch in range(self.max_iter):
            ...
            if self.log and epoch % 1000 == 0:
                print(classify_loss, consistency_loss, loss)
            loss.backward()
            optimizer.step()

            alpha = min(1.0 - 1.0 / (epoch + 1), 0.99)
